...
bool cmp(luogu_Sorting2 x, luogu_Sorting2 y)
{
    if (x.taka == y.taka)
        return x.pos < y.pos;
    return x.taka > y.taka;
}
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        p[i].taka = tk;
        sum += tk;
        error(p[i].name, p[i].pos, p[i].taka);
    }
    sort(all(p),cmp);
