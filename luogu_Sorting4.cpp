...
// *****************************************************
lli ans = 0;
void merge_1(vll &p,lli left,lli mid,lli right){
  
    lli n1=mid-left+1;
    lli n2=right-mid;
    vll l(n1),r(n2);
    for(lli i=0;i<n1;i++)l[i]=p[left+i];
    for(lli i=0;i<n2;i++)r[i]=p[mid+1+i];
    lli i=0,j=0,k=left;
    while(i<n1&&j<n2){
        if(l[i]<=r[j]){
            p[k++]=l[i++];
...
void merge_sort(vll &p, lli left, lli right)
{
