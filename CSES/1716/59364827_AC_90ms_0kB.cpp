...
// ------------------- CODE STARTS HERE ----------------
// *****************************************************
vector<lli> factorial(lli n = MAXN + 5, lli mod = mod)
{
...
lli nCr(lli n, lli r, const vector<lli> &fact, lli mod = mod)
{
    ...
    lli denominator = mmul(fact[r], fact[n - r], mod);
    return mmul(numerator, minv(denominator, mod), mod);
}
int main()
{
    ...
    tc0
    {
        lli n, m;
        cin >> n >> m;
        vll fact = factorial(n+m);
        cout << nCr(n + m - 1LL, m, fact) << "\n";
    }
}