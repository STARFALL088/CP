...
// ------------------- CODE STARTS HERE ----------------
// *****************************************************
vector<pair<lli, lli>> p(10000);
vvl dp;
// int rec(int x,int i){
//     if(x<0)return -INF;//invalid
//     if(i<=0)return 0;
...
//     lli a= rec(x,i-1);
//     //take
//     lli b= p[i].second+rec(x-p[i].first,i-1);

//     return dp[x][i]= max(a,b);
// }

// memorization er sathe space optimization possible na
// aage tabulation e newa lage
    ...
    tc0
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            for (int j = 0; j <= x; j++)
            {
                dp[j][i%2] = dp[j][(i - 1)%2];
                if (j >= p[i].first)
                    dp[j][i%2] = max(dp[j][i%2], dp[j - p[i].first][(i - 1)%2] + p[i].second);
                //cout << dp[j][i%2] << gp;
            }
            //nl
        }
