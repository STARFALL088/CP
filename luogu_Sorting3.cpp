...
vector<luogu_Sorting3>p(100001),win(100001),lose(100001);

bool cmp(luogu_Sorting3 x,luogu_Sorting3 y ){
       if(y.point!=x.point)return x.point>y.point;
        return x.idx<y.idx;
}
int main() {
    ...
    tc0 {
        ...
        for(int i=0;i<2*n;i++)p[i].idx=i+1;
        for(int i=0;i<2*n;i++)cin>>p[i].point;
        for(int i=0;i<2*n;i++)cin>>p[i].s;
        sort(all(p),cmp);
        //for(int _=0;_<2*n;_++)cerr<<p[_].idx<<gp;nl
        while(r--){
            ...
            merge((win).begin(), (win).end(),(lose).begin(), (lose).end(),p.begin(),cmp);
        }
        cout<<p[q-1].idx<<endl;
        //for(int _=0;_<2*n;_++)cerr<<p[_].idx<<gp;nl
    }
