...
vector<vector<ll>> dis(1006, vector<ll>(1006));
...
void bfs()
{
    ...
    // q.push({x, y});
    loop(i, n) loop(j, m) if (dis[i][j]==0) q.push({i, j});
    // vis[x][y] = true;
    ...
    while (!q.empty())
    {
        ...
        for (lli i = 0; i < 4; i++)
        {
            ...
            if (isValid(nx, ny)and dis[nx][ny]>dis[cx][cy]+1)
            {
