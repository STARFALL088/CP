...
// ------------------- CODE STARTS HERE ----------------
// *****************************************************
vector<vector<lli>> adj(MAXN);
vll dis(MAXN, 0);
vector<lli> visited(MAXN, false);
vector<lli> parent(MAXN, -1);
lli cycle_start = -1, cycle_end = -1;

bool dfs(lli u, lli par = -1)
{
    visited[u] = true;
    for (lli v : adj[u])
    {
...
vll get_cycle()
{
    ...
    for (lli v = cycle_end; v != cycle_start; v = parent[v])
        cycle.push_back(v);
    cycle.push_back(cycle_start);
