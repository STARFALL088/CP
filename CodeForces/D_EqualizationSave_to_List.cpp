...
int main()
{
    ...
    cin.tie(0);
    cout.tie(0);
    lli dp[61][61]; // dp(i,j,k)->dp(x e used,y e used, current use )
    ...
    for (int k = 0; k < 60; k++)
    {
        for (int i = 60; i >= 0; i--)
        {
            for (int j = 60; j >=0; j--)
            {
                if(dp[i][j]==INF)continue;//ar baraia laav nai
                if (i + k <= 60)
                    dp[i + k][j] = min(dp[i + k][j], dp[i][j] + (1LL << k)); //k knapsack
                if (j + k <= 60)
                    dp[i][j + k] = min(dp[i][j + k], dp[i][j] + (1LL << k)); //k knapsack
            }
        }
