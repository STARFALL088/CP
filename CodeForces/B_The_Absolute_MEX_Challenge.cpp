...
lli findMex(vector<lli> &a)
{
    ...
    // Find the first index that is not marked (false)
    for (lli mex = 0; mex <= n; mex++)
    {
        if (!present[mex])
        {
            return mex;
    ...
    // If all numbers from 0 to n are present, Mex is n+1
    return n + 1;
}
