...
int main()
{
    ...
    tc0{
        ...
        //print(p);
        for(int i=0; i<n; i++) {
            if(p[i]==p.back()/3){f++;pos_1=i;}
            if(p[i]==2*p.back()/3){s++;pos_2=i;}

        }
       if(pos_1>pos_2)f=0,s=0;

        if(p.back()==0)cout<<(f-1)*(f-2)/2<<endl;
        else {
            lli pre_cnt = 0;
            for (int i = 0; i < n - 1; i++) {
                if (p[i] == p.back() / 3) pre_cnt++;
                if (p[i] == 2 * p.back() / 3) cnt += pre_cnt;
            }
            cout << cnt << endl;
        }
    }
