...
#define tolower_str(s) trcntform(all(s), s.begin(), ::tolower)
#define toupper_str(s) trcntform(all(s), s.begin(), ::toupper)
#define cng_str(s) trcntform(all(s), s.begin(), [](char c) { return c ^ ' '; })
...
// Mission ladder clear... 1 din o tikbe na -_-
...
int main()
{
    ...
    tc
    {
        ...
        srt(p);
        // laga brute
        cout << max({p[0] * p[1] * p[2] * p[3] * p[4],
                     p[0] * p[1] * p[2] * p[3] * p[p.size() - 1],
                     p[0] * p[1] * p[2] * p[p.size() - 2] * p[p.size() - 1],
                     p[0] * p[1] * p[p.size() - 3] * p[p.size() - 2] * p[p.size() - 1],
                     p[0] * p[p.size() - 4] * p[p.size() - 3] * p[p.size() - 2] * p[p.size() - 1],
                     p[p.size() - 5] * p[p.size() - 4] * p[p.size() - 3] * p[p.size() - 2] * p[p.size() - 1]})
             << endl;
    }
}