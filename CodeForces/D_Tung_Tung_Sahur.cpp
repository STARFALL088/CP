...
int main() {
    ...
    tc {
        ...
        loop(i,p.size()){
            if(p[i]=='L'){
                if(i!=0 and cr!=0){a.pb(cr);cr=0;}
                cl++;
            }
            if(p[i]=='R'){
                if(i!=0 and cl!=0){a.pb(cl);cl=0;}
                cr++;
            }
        }
        if(cr)a.pb(cr);if(cl)a.pb(cl);
        cr=0,cl=0;
        loop(i,s.size()){
            if(s[i]=='L'){
                if(i!=0 and cr!=0){b.pb(cr);cr=0;}
                cl++;
            }
            if(s[i]=='R'){
                if(i!=0 and cl!=0){b.pb(cl);cl=0;}
                cr++;
            }
        }
        if(cr)b.pb(cr);if(cl)b.pb(cl);
        debug(a);
        debug(b);
        bool flag=0;
        loop(i,a.size()){
            if(b[i]>2*a[i] or b[i]<a[i]){
                flag=1;
                break;
            }
        }
