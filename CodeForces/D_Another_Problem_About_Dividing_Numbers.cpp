...
int main()
{
    ...
    for (int i = 0; i <= MX; i++) if (primes[i]) p.pb(i);
    tc
    {
        lli a, b, k;
        cin >> a >> b >> k;
        ...
        if (a == b)
        {
            ...
            mn = 2;
        lli mx = 0;
        map<lli, lli> m = prime_factorize(g);
        for (auto [i, j] : m)
        {
            mx += j;
        }
        ...
        if (a % b == 0)
        {
            map<lli, lli> m1 = prime_factorize(a / b);
            for (auto [i, j] : m1)
            {
                mx += j;
            }
        ...
        map<lli, lli> m1 = prime_factorize(a / g);
        for (auto [i, j] : m1)
        {
            mx += j;
        }
        map<lli, lli> m2 = prime_factorize(b / g);
        for (auto [i, j] : m2)
        {
            mx += j;
        }
