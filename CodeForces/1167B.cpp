...
// ------------------- CODE STARTS HERE ----------------
// *****************************************************
unordered_map<lli, pll> m;
lli ask(lli a,lli b){
    cout<<"? "<<a<<gp<<b<<endl;
    cout.flush();
    lli prod;
    cin>>prod;
    //cerr<<m[prod].first<<gp<<m[prod].second<<endl;
    return prod;
}
int main()
{
    ...
    tc0
    {
        ...
        vll p = {-1, 4, 8, 15, 16, 23, 42};
        loop(i, 6) loop(j, 6) if(m.count(p[(i+1)]*p[(j+1)])==0)m[{p[i + 1]* p[j + 1]}] = {p[(i + 1)] , p[(j + 1)]};
       

        lli ab=ask(1,2);
        lli bc=ask(2,3);
        if(m[ab].first==m[bc].second)b=m[ab].first;
        if(m[ab].first==m[bc].first)b=m[ab].first;
        if(m[ab].second==m[bc].first)b=m[ab].second;
        if(m[ab].second==m[bc].second)b=m[ab].second;
        a=ab/b;
        c=bc/b;

        lli de=ask(4,5);
        lli ef=ask(5,6);
        if(m[de].first==m[ef].second)e=m[de].first;
        if(m[de].first==m[ef].first)e=m[de].first;
        if(m[de].second==m[ef].first)e=m[de].second;
        if(m[de].second==m[ef].second)e=m[de].second;
        d=de/e;
        f=ef/e;
        cout<<"! "<<a<<gp<<b<<gp<<c<<gp<<d<<gp<<e<<gp<<f<<endl;
        cout.flush();
