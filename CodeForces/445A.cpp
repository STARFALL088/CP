...
// ------------------- CODE STARTS HERE ----------------
// *****************************************************
vector<vector<bool>> vis(200, vb(200, 0));
lli n, m;
...
void bfs(int x, int y, vector<vector<char>> &p)
{
    ...
    while (!q.empty())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                vis[nx][ny] = true;
                if (p[cx][cy] == 'W' && p[nx][ny] == '.')
                {
                    p[nx][ny] = 'B';
                }
                else if (p[cx][cy] == 'B' && p[nx][ny] == '.')
                {
                    p[nx][ny] = 'W';
                }
    ...
    tc0
    {
        ...
        for (auto i : action)
            if (p[i.first][i.second] == '.')
                p[i.first][i.second] = 'B', bfs(i.first, i.second, p);
        for (int i = 0; i < n; i++)
            for (int j = 0; j < m; j++)
                cout << p[i][j] << (j == m - 1 ? "\n" : "");
    }
