...
#define debug(x)              \
    ...
    for (const auto &val : x) \
        cerr << val << " ";   \
    cerr << "]\n"; // it works fine for now ig,,,,I'll deal with you later
#define take(p, n, T) \
    vector<T> p(n);   \
    for (auto &i : p) \
        cin >> i;
...
#define lims(x, l, r) (x = max(l, min(x, r)))    // limit e aane
#define isin(x, l, r) ((l) <= (x) && (x) <= (r)) // depends
#define pcnt __builtin_popcountll
#define snuke srand((unsigned)clock() + (unsigned)time(NULL)) // eda abar ki
...
// Graph input macro......eda r use korsi-_-
...
int main()
{
    ...
    tc0
    {
       ...
       if(n==1)cnt=1;
        for (int i = 1; i < n-1; i++)
        {
            if (p[i].s < p[i].f - p[i - 1].f)
            {
                cnt++;
                continue;
            }
            if (p[i].s < p[i+1].f - p[i].f)
            {
                cnt++;
                p[i].f += p[i].s;continue;
            }
        }
        cout<<cnt<<endl;
    }
