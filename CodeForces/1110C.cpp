...
int main()
{
    ...
    tc
    {
        ...
        // lli g=0;
        //  for(int i=1;i<a;i++){
        //      g=maxx(g,gcd((a^i),(a&i)));
        //      error((a^i),(a&i),g);

        // now since 11111111111111111111
        //(a^i)+(a^i)=a  form error :)
        // so ,have to break a into 2 parts so that g is max
        // but kemne?????????//

        // sqrt n prime fact??????

        // largest factor (a baade ofc)??
