...
void build(int idx, int low, int high, int arr[], int seg[], int level)
{
    if (low == high)
    {
        seg[idx] = arr[low];
    ...
    int mid = (low + high) >> 1;
    build(2 * idx + 1, low, mid, arr, seg, level + 1);
    build(2 * idx + 2, mid + 1, high, arr, seg, level + 1);
    if ((x-level) % 2)
        seg[idx] = seg[2 * idx + 1] | seg[2 * idx + 2];
    else
        seg[idx] = seg[2 * idx + 1] ^ seg[2 * idx + 2];
}
void update(int idx, int low, int high, int pos, int val, int seg[], int level)
{
    if (low == high)
    {
        seg[idx] = val;
    ...
    int mid = (low + high) >> 1;
    if (pos <= mid)
        update(2 * idx + 1, low, mid, pos, val, seg, level + 1);
    else
        update(2 * idx + 2, mid + 1, high, pos, val, seg, level + 1);
    if ((x-level) % 2)
        seg[idx] = seg[2 * idx + 1] | seg[2 * idx + 2];
    else
        seg[idx] = seg[2 * idx + 1] ^ seg[2 * idx + 2];
