...
void precomputeFactorials()
{
    ...
    invFact[MX - 1] = mpow(fact[MX - 1], mod - 2); // Fermat's Little Theorem
    for (int i = MX - 2; i >= 0; i--)
        invFact[i] = (invFact[i + 1] * (i + 1)) % mod;
}

// Combination function with modular arithmetic
lli c(lli n, lli r)
{
    if (r > n)
        return 0;
...
int main()
{
    ...
    tc
    {
        ...
        lli zero = n - one;
        lli lage = (k + 1) / 2;
        for (lli r = lage; r <= one && r <= k; r++){
            ans = (ans + c(one, r) * c(zero, k - r) % mod) % mod;
        }
        cerr << "ans-->";
        cout << ans % mod << endl;
    }
