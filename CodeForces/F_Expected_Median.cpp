...
lli f(lli n, lli r, const vector<lli> &fact, lli mod = mod)
{
    if (r > n)
        return 0;
...
int main()
{
    ...
    tc
    {
        ...
        lli ans = 0;
        for (int i = use; i <= min(k, one); i++)
        {
            lli x = f(one, i, fact)%mod;
            error(x, one, i);
            ans = madd(ans, mmul(x, f(zero, k - i, fact)));
        }
