...
// ------------------- CODE STARTS HERE ----------------
// *****************************************************
bool comp(const tuple<lli, lli, char> &a, const tuple<lli, lli, char> &b)
{
    if (get<0>(a) != get<0>(b))
    {
...
int main()
{
    ...
    tc0
    {
        string s;
        cin >> s;
        vector<tuple<lli, lli, char>> p;
        int as = 0;
        for (int i = 0; i < s.size(); i++)
        {
            // error(as,i+1,s[i])
            p.push_back({as, i + 1, s[i]});
        ...
        for (auto i : p)cout<<get<2>(i);
            //cout << get<0>(i)<<get<1>(i)<<gp;
    }
}