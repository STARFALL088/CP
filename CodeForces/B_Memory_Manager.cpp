    ...
    tc0 {
        ...
        loop(i,t){
            ...
            if(s=="alloc"){
                ...
                lli cur=ide;
                lli z=-1;
                lli capacity=0,zero=0;
                debug(p);
                loop(i,m){
                    if(p[i]==-1000000000000) ++zero;
                    else {capacity=max(zero,capacity);zero=0;}
                }
                capacity=max(zero,capacity);zero=0;
                error(capacity);
                if(capacity<n){
                    cout<<"NULL"<<endl;
                    continue;
                }
                loop(i,m)if(p[i]==-1000000000000){
                    z=i;
                    break;
                }
                for(int i=z;i<z+n;i++){
                ...
                loop(i,m){
                    if(p[i]==x){p[i]=-1000000000000;cnt++;}
                    
                }
                if(cnt==0)cout<<"ILLEGAL_ERASE_ARGUMENT"<<endl;
                    

            }
            else{
                lli j=0;
                vll q(m,-1000000000000);
                loop(i,m)if(p[i]!=-1000000000000)q[j++]=p[i];
                p=q;
            }
        }
    }
