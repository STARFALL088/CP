...
namespace io
{
    ...
    template <typename K, typename V>
    istream &operator>>(istream &is, map<K, V> &m)
    {
        size_t n;
        is >> n;
        while (n--)
...
const unsigned long long INF = 1001001001;
const unsigned long long MX = 200005;
...
#define lg(r, n) (unsigned long long)(log2(n) / log2(r))
...
// Loop macros
#define loop(i, n) for (unsigned long long i = 0; i < (n); ++i)
#define rloop(i, n) for (unsigned long long i = (n) - 1; i >= 0; --i)
#define sloop(i, s, t) for (unsigned long long i = s; i < t; ++i)
...
#define tc    \
    ...
    cin >> t; \
    for (unsigned long long ii = 1; ii <= t; ++ii)
#define tc0    \
...
int main()
{
    ...
    tc
    {
        ...
        for (unsigned long long i = 0; i < n; i++)
        {
            if (p.size() == n)
                break;
            if (((x | i) == x) && (binary_search(p.begin(),p.begin()+_, i) == 0))
                p.pb(i);
        }
        while (p.size() < n)
        {
