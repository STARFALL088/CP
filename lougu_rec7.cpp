...
// ------------------- CODE STARTS HERE ----------------
// *****************************************************
int a[5001]={0},b[5001]={0},c[5001]={0};
int main() {
    ...
    tc0{
        ...
        for(int i=2;i<=n;i++){
           ...
           int haate=0;
           while(j--){
            // error(a[5000],b[5000],j);break;
            int ase=a[j]+b[j]+haate;
            // error(ase);break;
            haate=ase/10;
            ase%=10;
            c[j]=ase;
           ...
           for(int j=0;j<5001;j++) a[j]=b[j];
           for(int j=0;j<5001;j++) b[j]=c[j];
        }
        int on=0;
        for(int i=0;i<=5000;i++){if(c[i])on=1;
        if(on)cout<<c[i];}
        if(n==1)cout<<n;nl
    }
}