...
int main()
{
    ...
    while (cin >> s)
    {
        ...
        loop(i, n)
        {
            if (st.empty())
                mx1 = 0;
            if ((s[i] == r[0] and st.empty()) or (s[i] == r[1] and st.empty()) or (s[i] == r[2] and st.empty()))
            {
                ans = max(ans, mx1);
                mx1 = 0;
            ...
            if (s[i] == l[0] or s[i] == l[1] or s[i] == l[2])
                st.push(s[i]);
            else
            {
                if ((s[i] == r[0] and st.top() == l[0]) or (s[i] == r[1] and st.top() == l[1]) or (s[i] == r[2] and st.top() == l[2]))
                {
                    mx1 += 2;
                    ans = max(ans, mx1);
                    st.pop();
