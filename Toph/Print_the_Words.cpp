...
int main() {
    ...
    tc0 {
        ...
        while(cin>>_s){
            if(_s.back()==','||_s.back()=='.'||_s.back()==';')_s.pop_back();
            string tmp=_s;
            for(auto &i:_s) if(isin(i,'A','Z'))i=i-'A'+'a';
            if(_s.size())if(last.count(_s)==0)last[_s]=tmp;
            if(_s.size())s.pb(last[_s]);
        }
        
        a=b=c=s[0];
        for(auto i:s){
            m[i]++;
            if(mx[0]<i.size())a=i,mx[0]=i.size();
            if(mx[1]>i.size())b=i,mx[1]=i.size();
            if(mx[2]<m[i] or (m[i] == mx[2] && c > i))c=i,mx[2]=m[i];
