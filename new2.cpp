...
int main() {
    ...
    tc0 {
        ...
        lld x3=(r1*x2+(d-r1)*x1)/(d);
        lld y3=(r1*y2+(d-r1)*y1)/d;
        lld x4=(r2*x1+(d-r2)*x2)/(d);
        lld y4=(r2*y1+(d-r2)*y2)/d;
        cout<<setpre(15)<<(x3+x4)/2<<gp<<(y3+y4)/2<<endl;
    }
}