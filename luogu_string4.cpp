...
bool valid(string s){
    if(s=="92200229")return 1;
    int month=stoi(s.substr(4,2));
    if(!isin(month,1,12))return 0;
    int date=stoi(s.substr(6,2));
    if(isin(date,1,p[month]))return 1;
return 0;
}
string f(string a)
{
    string new_a = a.substr(0, 4);
    new_a += new_a;
    reverse(4 + all(new_a));
     cerr<<new_a<<endl;
    return new_a;
...
int main()
{
    ...
    tc0
    {
        string a, b;
        cin >> a >> b;
        int start=stoi(a.substr(0,4));
        int end=stoi(b.substr(0,4));
       
        int cnt=0;
        for(int i=start;i<=end;i++){
            if(valid(f(to_string(i))))cnt++;
             cerr<<i<<endl;
             error(cnt);
        }
        cnt-=(f(to_string(start))<a && valid(f(to_string(start))));
        cnt-=(f(to_string(end))>b && valid(f(to_string(end))));
